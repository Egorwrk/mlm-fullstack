...
class Registration
{
    ...
    public function registration()
    {
        ...
        try {
            ...
            if (empty($datauser)) {
                $stmt = $this->connect->prepare('INSERT INTO users_auth (login, hash_password, email) VALUES (:login, :hash_password, :email)');
                $stmt->execute(array(':login' => $login, ':hash_password' => md5($password), ':email' => $email));
                $stmtuser = $this->connect->prepare('SELECT * FROM users_auth WHERE login = :login');
